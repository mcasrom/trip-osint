...
<div class='trip-header'>
    <h1>{pais['emoji']} {pais_nombre} &nbsp;
        <span class='risk-badge risk-{nivel_riesgo}'>{emoji_r} {label_r}</span>
    </h1>
    <p>{pais['capital']} · {pais['region']} ·
       {pais['moneda_nombre']} ({pais['moneda_codigo']}) ·
       {pais.get('idioma','N/D')} ·
       🔌 {pais.get('enchufes','N/D')} · {pais.get('voltaje','N/D')} ·
       🚗 {pais.get('conduccion_lado','Derecha')} &nbsp;·&nbsp;
       Motivo: <b style='color:#c8d8e8'>{motivo}</b>
    </p>
</div>
""", unsafe_allow_html=True)
...
tab_riesgo, tab_maec, tab_meteo, tab_divisa, tab_requisitos, tab_salud, tab_prensa, tab_checklist, tab_info = tabs
